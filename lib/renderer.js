...
function shader_vars_initialize(prog) {
    ...
    litPos_location = gl.getUniformLocation(prog, "lit_pos"); 
    ...
    fran_test_location = gl.getUniformLocation(prog, "fran_test"); 
...
function shader_vars_apply(timeStamp) {
    ...
    if (sunDir_location) { 
        let thingy = Mat4.rotation_xy(0.1).mul(Mat4.rotation_xz(0.75)).basis_z(); 
        //gl.uniform3f(sunDir_location, thingy.x, thingy.y, thingy.z); 
        gl.uniform3f(sunDir_location, -1, 0, 0); 
    }
    if(litPos_location) {
        let thingy = ShipMotor.player_ship.node.model.position().add(ShipMotor.player_ship.node.model.basis_z().scaled(-1.5));
        gl.uniform3f(litPos_location, thingy.x, thingy.y, thingy.z);
    }
    ...
    if(fran_test_location) {
        // engine light
        let light = 2 * ((Math.random() * 0.05) + (Math.random() * 0.05) + (Math.random() * 0.05) + (Math.random() * 0.05));
        if(ShipMotor.player_ship != null) {
            light *= ShipMotor.player_ship.throttle;
        }
        gl.uniform1f(fran_test_location, light);
        //gl.uniform1f(fran_test_location, document.getElementById("fran_tst").value);
    }
