...
const skyboxProgram = create_compile_and_link_program(gl, skybox_vertex_source, skybox_fragment_source);
...
function renderLoop(timeStamp) { 
    ...
    // Render the skybox
    renderSkybox(viewMatrix, projectionMatrix, skyboxTexture);
...
function renderSkybox(viewMatrix, projectionMatrix, skyboxTexture) {
    gl.useProgram(skyboxProgram);

    // Set view and projection matrices
    const viewLocation = gl.getUniformLocation(skyboxProgram, "view");
    const projectionLocation = gl.getUniformLocation(skyboxProgram, "projection");
    const skyboxLocation = gl.getUniformLocation(skyboxProgram, "skybox");

    // Remove translation from the view matrix
    const viewWithoutTranslation = viewMatrix.clone();
    viewWithoutTranslation[12] = 0;
    viewWithoutTranslation[13] = 0;
    viewWithoutTranslation[14] = 0;

    gl.uniformMatrix4fv(viewLocation, false, viewWithoutTranslation);
    ...
    const coord = gl.getAttribLocation(skyboxProgram, "coordinates");
