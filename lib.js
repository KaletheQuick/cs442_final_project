...
function xor_texture(){
    ...
    // console.log (data [100*1024+(100 * 4)]);
}
return new Uint8Array(data);

}

function create_and_load_texture( image ){
...
function create_and_load_elements_buffer(gl, indices, usage) {
    // Get the current element array buffer binding
    let current_element_buf = gl.getParameter(gl.ELEMENT_ARRAY_BUFFER_BINDING);
    
    let buf_id = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buf_id);
    
    //  the indices array is converted to a Uint16Array since index values are typically 16-bit integers.
    ...
    return buf_id; // Return the buffer ID
}
...
function set_uniform_vec3( gl, program, name, data )
{
    const loc = gl.getUniformLocation( program, name );
    if( loc == -1 ) {
        throw new Error( 'either no attribute named "' + attrib_name +
...
function set_uniform_vec4( gl, program, name, data )
{
    const loc = gl.getUniformLocation( program, name );
    if( loc == -1 ) {
        throw new Error( 'either no attribute named "' + attrib_name +
